...
int main(int argc, char **argv) {
	// open BMX.exe and dump contents
	FILE *f = fopen("BMX.exe", "rb");
	...
	if (f) {
		...
		if (filesize == 217088) {
			...
			printf("Make sure MHPB original release is installed\n");
			goto end;
		}
		...
		if (buffer) {
			...
			// write to THPS3.exe
			printf("Creating MHPB.exe\n");
			FILE *fout = fopen("MHPB.exe", "wb");
			if (fout) {
